...
def run_closed_trades_data_process(event,context):
    ...
    lambda_signifier = payload[0]['lambda_signifier']
    if lambda_signifier == "default_value":
        lambda_signifier = retrieve_signifier()
    logger.info(f"LAMBDA SIGNIFIER: {lambda_signifier}")

    succesful_logs, unsuccesful_logs = pull_log_data_closed("closed",lambda_signifier)
    ...
    s3.put_object(Bucket=trading_data_bucket, Key=f"failed_close_orders_data/{env}/{datetime.now(est).strftime('%Y/%m/%d/%H/%M')}.csv", Body=failed_df.to_csv(index=False))
    s3.put_object(Bucket=trading_data_bucket, Key=f"successful_close_orders_data/{env}/{datetime.now(est).strftime('%Y/%m/%d/%H/%M')}.csv", Body=success_df.to_csv(index=False))
