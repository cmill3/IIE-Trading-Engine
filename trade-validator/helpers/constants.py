CALL_STRATEGIES = ["IDXC","IDXC_1d","MA","MA_1d","GAIN_1d","GAIN","LOSERSC","LOSERSC_1d","VDIFFC","VDIFFC_1d","CDGAIN","CDGAIN_1d","CDMA","CDMA_1d",
                   "CDLOSEC","CDLOSEC_1d","CDBFC","CDBFC_1D",'CDHVC','CDHVC_1D','CDPRICEDIFFC_1D','CDPRICEDIFFC','CDVOLDIFFC_1D','CDVOLDIFFC',
                   'EH_BFC_1D_ALL','EH_BFC_1D_AMPM','EH_BFC_1D_AM','EH_BFC_1D_PM','EH_BFC_ALL','EH_BFC_AMPM','EH_BFC_AM','EH_BFC_PM']

PUT_STRATEGIES = ["IDXP","IDXP_1d","MAP","MAP_1d","GAINP_1d","GAINP","LOSERS","LOSERS_1d","VDIFFP","VDIFFP_1d","CDLOSE","CDLOSE_1d","CDMAP","CDMAP_1d",
                  "CDGAINP","CDGAINP_1d","CDBFP","CDBFP_1D",'CDHVP','CDHVP_1D','CDPRICEDIFFP_1D','CDPRICEDIFFP','CDVOLDIFFP','CDVOLDIFFP_1D',
                  'EH_BFP_1D_ALL','EH_BFP_1D_AMPM','EH_BFP_1D_AM','EH_BFP_1D_PM','EH_BFP_ALL','EH_BFP_AMPM','EH_BFP_AM','EH_BFP_PM']

ONED_STRATEGIES = ["IDXC_1d","IDXP_1d","MA_1d","MAP_1d","GAIN_1d","GAINP_1d","LOSERS_1d","LOSERSC_1d","VDIFFC_1d","VDIFFP_1d","CDGAIN_1d","CDMA_1d","CDLOSEC_1d","CDLOSE_1d",
                   "CDMAP_1d","CDGAINP_1d","CDBFC_1D","CDBFP_1D",'CDHVC_1D','CDHVP_1D','CDPRICEDIFFP_1D','CDPRICEDIFFC_1D','CDVOLDIFFP_1D','CDVOLDIFFC_1D',
                   'EH_BFC_1D_ALL','EH_BFP_1D_ALL','EH_BFC_1D_AMPM','EH_BFP_1D_AMPM','EH_BFC_1D_AM','EH_BFP_1D_AM','EH_BFC_1D_PM','EH_BFP_1D_PM']

THREED_STRATEGIES = ["IDXC","IDXP","MA","MAP","GAIN","GAINP","LOSERS","LOSERSC","VDIFFC","VDIFFP","CDGAIN","CDMA","CDLOSEC",
                    "CDLOSE","CDMAP","CDGAINP","CDBFC","CDBFP",'CDHVC','CDHVP','CDPRICEDIFFC','CDPRICEDIFFP','CDVOLDIFFP','CDVOLDIFFC',
                    'EH_BFC_ALL','EH_BFP_ALL','EH_BFC_AMPM','EH_BFP_AMPM','EH_BFC_AM','EH_BFP_AM','EH_BFC_PM','EH_BFP_PM']

ALGORITHM_CONFIG = {
    "GAIN_1D": {
        "target_label": "one_max",
        "target_value": .0175,
    },
    "GAIN": {
        "target_label": "three_max",
        "target_value": .0275,
    },
    "GAINP_1D": {
        "target_label": "one_min",
        "target_value": -.018,
    },
    "GAINP": {
        "target_label": "three_min",
        "target_value": -.028,
    },
     "LOSERSC_1D": {
        "target_label": "one_max",
        "target_value": .016,
    },
    "LOSERSC": {
        "target_label": "three_max",
        "target_value": .025,
    },
    "LOSERS_1D": {
        "target_label": "one_min",
        "target_value": -.013,
    },
    "LOSERS": {
        "target_label": "three_min",
        "target_value": -.02,
    },
    "vdiffC1d": {
        "target_label": "one_max",
        "target_value": .013,
    },
    "vdiffC": {
        "target_label": "three_max",
        "target_value": .028,
    },
    "vdiffP1d": {
        "target_label": "one_min",
        "target_value": -.013,
    },
    "vdiffP": {
        "target_label": "three_min",
        "target_value": -.028,
    },
     "MA_1D": {
        "target_label": "one_max",
        "target_value": .013,
    },
    "MA": {
        "target_label": "three_max",
        "target_value": .028,
    },
    "MAP_1D": {
        "target_label": "one_min",
        "target_value": -.013,
    },
    "MAP": {
        "target_label": "three_min",
        "target_value": -.028,
    },
    "idxC1d": {
        "target_label": "one_max",
        "target_value": .013,
    },
    "idxC": {
        "target_label": "three_max",
        "target_value": .028,
    },
    "idxP1d": {
        "target_label": "one_min",
        "target_value": -.013,
    },
    "idxP": {
        "target_label": "three_min",
        "target_value": -.028,
    },
}

YEAR_CONFIG = {
    "twenty1": {
        "months":
            [
            ['2021-01-04', '2021-01-11', '2021-01-18', '2021-01-25'],
            ['2021-02-01', '2021-02-08', '2021-02-15', '2021-02-22'],
            ['2021-03-01', '2021-03-08', '2021-03-15','2021-03-22', '2021-03-29'],
            ['2021-04-05', '2021-04-12', '2021-04-19', '2021-04-26'],
            [ '2021-05-03', '2021-05-10', '2021-05-17', 
            '2021-05-24', '2021-05-31'],
            ['2021-06-07', '2021-06-14', '2021-06-21', '2021-06-28'],
            ['2021-07-05', '2021-07-12','2021-07-19', '2021-07-26'],
            ['2021-08-02', '2021-08-09', '2021-08-16', '2021-08-23', '2021-08-30'],
            ['2021-09-06', '2021-09-13', '2021-09-20', '2021-09-27'],
            ['2021-10-04', '2021-10-11', '2021-10-18', '2021-10-25'],
            ['2021-11-01', '2021-11-08', '2021-11-15', '2021-11-22', '2021-11-29'],
            ['2021-12-06', '2021-12-13', '2021-12-20', '2021-12-27'],
            ],
        "all_files":
        [
                '2021-01-04', '2021-01-11', '2021-01-18', '2021-01-25', '2021-02-01', 
                '2021-02-08', '2021-02-15', '2021-02-22', '2021-03-01', '2021-03-08', 
                '2021-03-15','2021-03-22', '2021-03-29', '2021-04-05', '2021-04-12',
                '2021-04-19', '2021-04-26', '2021-05-03', '2021-05-10', '2021-05-17',
                '2021-05-24', '2021-05-31', '2021-06-07', '2021-06-14', '2021-06-21',
                '2021-06-28', '2021-07-05', '2021-07-12', '2021-07-19', '2021-07-26',
                '2021-08-02', '2021-08-09', '2021-08-16', '2021-08-23', '2021-08-30',
                '2021-09-06', '2021-09-13', '2021-09-20', '2021-09-27', '2021-10-04',
                '2021-10-11', '2021-10-18', '2021-10-25', '2021-11-01', '2021-11-08',
                '2021-11-15', '2021-11-22', '2021-11-29', '2021-12-06', '2021-12-13',
                '2021-12-20', '2021-12-27'
        ],
        "year": "21"
    },
    "twenty2": {
        "months":
            [
            ['2022-01-03', '2022-01-10', '2022-01-17', '2022-01-24', 
            '2022-01-31'],
            ['2022-02-07', '2022-02-14', '2022-02-21', '2022-02-28'],
            ['2022-03-07', '2022-03-14', '2022-03-21','2022-03-28'],
            ['2022-04-04', '2022-04-11', '2022-04-18', '2022-04-25'],
            [ '2022-05-02', '2022-05-09', '2022-05-16', 
            '2022-05-23', '2022-05-30'],
            ['2022-06-06', '2022-06-13', '2022-06-20', '2022-06-27'],
            ['2022-07-04', '2022-07-11','2022-07-18', '2022-07-25'],
            ['2022-08-01', '2022-08-08', '2022-08-15', '2022-08-22', '2022-08-29'],
            ['2022-09-05', '2022-09-12', '2022-09-19', '2022-09-26'],
            ['2022-10-03', '2022-10-10', '2022-10-17', '2022-10-24', '2022-10-31'],
            ['2022-11-07', '2022-11-14', '2022-11-21', '2022-11-28'],
            ['2022-12-05', '2022-12-12', '2022-12-19', '2022-12-26'],
            ],
        "all_files":
        [
                '2022-01-03', '2022-01-10', '2022-01-17', '2022-01-24','2022-01-31', 
                '2022-02-07', '2022-02-14', '2022-02-21', '2022-02-28', '2022-03-07', 
                '2022-03-14', '2022-03-21', '2022-03-28', '2022-04-04', '2022-04-11',
                '2022-04-18', '2022-04-25', '2022-05-02', '2022-05-09', '2022-05-16',
                '2022-05-23', '2022-05-30', '2022-06-06', '2022-06-13', '2022-06-20',
                '2022-06-27', '2022-07-04', '2022-07-11', '2022-07-18', '2022-07-25',
                '2022-08-01', '2022-08-08', '2022-08-15', '2022-08-22', '2022-08-29',
                '2022-09-05', '2022-09-12', '2022-09-19', '2022-09-26', '2022-10-03',
                '2022-10-10', '2022-10-17', '2022-10-24', '2022-10-31', '2022-11-07',
                '2022-11-14', '2022-11-21', '2022-11-28', '2022-12-05', '2022-12-12',
                '2022-12-19', '2022-12-26'
        ],
        "year": "22"
    },
    "twenty3": {
        "months":
            [
            ['2023-01-02', '2023-01-09', '2023-01-16', '2023-01-23', 
            '2023-01-30'],
            ['2023-02-06', '2023-02-13', '2023-02-20', '2023-02-27'],
            ['2023-03-06', '2023-03-13', '2023-03-20','2023-03-27'],
            ['2023-04-03', '2023-04-10', '2023-04-17', '2023-04-24'],
            [ '2023-05-01', '2023-05-08', '2023-05-15', 
            '2023-05-22', '2023-05-29'],
            ['2023-06-05', '2023-06-12', '2023-06-19', '2023-06-26'],
            ['2023-07-03', '2023-07-10','2023-07-17', '2023-07-24', '2023-07-31'],
            ['2023-08-07', '2023-08-14', '2023-08-21', '2023-08-28'],
            ['2023-09-04', '2023-09-11', '2023-09-18', '2023-09-25'],
            ['2023-10-02', '2023-10-09', '2023-10-16', '2023-10-23', '2023-10-30'],
            ['2023-11-06', '2023-11-13', '2023-11-20', '2023-11-27'],
            ['2023-12-04', '2023-12-11', '2023-12-18'],
            ],
        "all_files":
        [
                '2023-01-02', '2023-01-09', '2023-01-16', '2023-01-23', 
                '2023-01-30', '2023-02-06', '2023-02-13', '2023-02-20', '2023-02-27', '2023-03-06', '2023-03-13', '2023-03-20', 
                '2023-03-27', '2023-04-03', '2023-04-10', '2023-04-17', '2023-04-24', '2023-05-01', '2023-05-08', '2023-05-15', 
                '2023-05-22', '2023-05-29', '2023-06-05', '2023-06-12', '2023-06-19', '2023-06-26', '2023-07-03', '2023-07-10', 
                '2023-07-17', '2023-07-24', '2023-07-31', '2023-08-07', '2023-08-14', '2023-08-21', '2023-08-28', '2023-09-04', 
                '2023-09-11', '2023-09-18', '2023-09-25', '2023-10-02', '2023-10-09', '2023-10-16', '2023-10-23', '2023-10-30',
                '2023-11-06', '2023-11-13', '2023-11-20', '2023-11-27', '2023-12-04', '2023-12-11', '2023-12-18', '2023-12-25'
        ],
        "year": "23"
    },
    "twenty4": {
        "months":
            [
            ['2024-01-01', '2024-01-08', '2024-01-15', '2024-01-22', 
            '2024-01-29'],
            ['2024-02-05', '2024-02-12', '2024-02-19', '2024-02-26'],
            ['2024-03-04', '2024-03-11', '2024-03-18','2024-03-25'],
            ['2024-04-01', '2024-04-08', '2024-04-15', '2024-04-22', '2024-04-29'],
            [ '2024-05-06', '2024-05-13', '2024-05-20', 
            '2024-05-27'],
            ['2024-06-03', '2024-06-10', '2024-06-17', '2024-06-24'],
            ['2024-07-01', '2024-07-08','2024-07-15', '2024-07-22', '2024-07-29'],
            ['2024-08-05', '2024-08-12', '2024-08-19', '2024-08-26'],
            ['2024-09-02', '2024-09-09', '2024-09-16', '2024-09-23', '2024-09-30'],
            ['2024-10-07', '2024-10-14', '2024-10-21', '2024-10-28'],
            ['2024-11-04', '2024-11-11', '2024-11-18', '2024-11-25'],
            ['2024-12-02', '2024-12-09', '2024-12-16', '2024-12-23', '2024-12-30'],
            ],
        "all_files":[
                '2024-01-01', '2024-01-08', '2024-01-15', '2024-01-22', 
                '2024-01-29', '2024-02-05', '2024-02-12', '2024-02-19', '2024-02-26', '2024-03-04', '2024-03-11', '2024-03-18', 
                '2024-03-25', '2024-04-01', '2024-04-08', '2024-04-15', '2024-04-22', '2024-04-29', '2024-05-06', '2024-05-13', 
                '2024-05-20', '2024-05-27', '2024-06-03', '2024-06-10', '2024-06-17', '2024-06-24', '2024-07-01', '2024-07-08', 
                '2024-07-15', '2024-07-22', '2024-07-29', '2024-08-05', '2024-08-12', '2024-08-19', '2024-08-26', '2024-09-02', 
                '2024-09-09', '2024-09-16', '2024-09-23', '2024-09-30', '2024-10-07', '2024-10-14', '2024-10-21', '2024-10-28',
                '2024-11-04', '2024-11-11', '2024-11-18', '2024-11-25', '2024-12-02', '2024-12-09', '2024-12-16', '2024-12-23', '2024-12-30'    
        ],
        "year": "24"
    }
}


# YEAR_CONFIG = {
#     "twenty1": {
#         "months":
#             {
#             "m1":['2021-01-04', '2021-01-11', '2021-01-18', '2021-01-25'],
#             "m2":['2021-02-01', '2021-02-08', '2021-02-15', '2021-02-22'],
#             "m3":['2021-03-01', '2021-03-08', '2021-03-15','2021-03-22', '2021-03-29'],
#             "m4":['2021-04-05', '2021-04-12', '2021-04-19', '2021-04-26'],
#             "m5":[ '2021-05-03', '2021-05-10', '2021-05-17', 
#             '2021-05-24', '2021-05-31'],
#             "m6":['2021-06-07', '2021-06-14', '2021-06-21', '2021-06-28'],
#             "m7":['2021-07-05', '2021-07-12','2021-07-19', '2021-07-26'],
#             "m8":['2021-08-02', '2021-08-09', '2021-08-16', '2021-08-23', '2021-08-30'],
#             "m9":['2021-09-06', '2021-09-13', '2021-09-20', '2021-09-27'],
#             "m10":['2021-10-04', '2021-10-11', '2021-10-18', '2021-10-25'],
#             "m11":['2021-11-01', '2021-11-08', '2021-11-15', '2021-11-22', '2021-11-29'],
#             "m12":['2021-12-06', '2021-12-13', '2021-12-20', '2021-12-27'],
#         },
#         "all_files":
#         [
#                 '2021-01-04', '2021-01-11', '2021-01-18', '2021-01-25', '2021-02-01', 
#                 '2021-02-08', '2021-02-15', '2021-02-22', '2021-03-01', '2021-03-08', 
#                 '2021-03-15','2021-03-22', '2021-03-29', '2021-04-05', '2021-04-12',
#                 '2021-04-19', '2021-04-26', '2021-05-03', '2021-05-10', '2021-05-17',
#                 '2021-05-24', '2021-05-31', '2021-06-07', '2021-06-14', '2021-06-21',
#                 '2021-06-28', '2021-07-05', '2021-07-12', '2021-07-19', '2021-07-26',
#                 '2021-08-02', '2021-08-09', '2021-08-16', '2021-08-23', '2021-08-30',
#                 '2021-09-06', '2021-09-13', '2021-09-20', '2021-09-27', '2021-10-04',
#                 '2021-10-11', '2021-10-18', '2021-10-25', '2021-11-01', '2021-11-08',
#                 '2021-11-15', '2021-11-22', '2021-11-29', '2021-12-06', '2021-12-13',
#                 '2021-12-20', '2021-12-27'
#         ]
#     },
#     "twenty2": {
#         "months":
#             {
#             "m1":['2022-01-03', '2022-01-10', '2022-01-17', '2022-01-24', 
#             '2022-01-31'],
#             "m2":['2022-02-07', '2022-02-14', '2022-02-21', '2022-02-28'],
#             "m3":['2022-03-07', '2022-03-14', '2022-03-21','2022-03-28'],
#             "m4":['2022-04-04', '2022-04-11', '2022-04-18', '2022-04-25'],
#             "m5":[ '2022-05-02', '2022-05-09', '2022-05-16', 
#             '2022-05-23', '2022-05-30'],
#             "m6":['2022-06-06', '2022-06-13', '2022-06-20', '2022-06-27'],
#             "m7":['2022-07-05', '2022-07-11','2022-07-18', '2022-07-25'],
#             "m8":['2022-08-01', '2022-08-08', '2022-08-15', '2022-08-22', '2022-08-29'],
#             "m9":['2022-09-05', '2022-09-12', '2022-09-19', '2022-09-26'],
#             "m10":['2022-10-03', '2022-10-10', '2022-10-17', '2022-10-24', '2022-10-31'],
#             "m11":['2022-11-07', '2022-11-14', '2022-11-21', '2022-11-28'],
#             "m12":['2022-12-05', '2022-12-12', '2022-12-19', '2022-12-26'],
#         },
#         "all_files":
#         [
#                 '2022-01-03', '2022-01-10', '2022-01-17', '2022-01-24','2022-01-31', 
#                 '2022-02-07', '2022-02-14', '2022-02-21', '2022-02-28', '2022-03-07', 
#                 '2022-03-14', '2022-03-21', '2022-03-28', '2022-04-04', '2022-04-11',
#                 '2022-04-18', '2022-04-25', '2022-05-02', '2022-05-09', '2022-05-16',
#                 '2022-05-23', '2022-05-30', '2022-06-06', '2022-06-13', '2022-06-20',
#                 '2022-06-27', '2022-07-05', '2022-07-11', '2022-07-18', '2022-07-25',
#                 '2022-08-01', '2022-08-08', '2022-08-15', '2022-08-22', '2022-08-29',
#                 '2022-09-05', '2022-09-12', '2022-09-19', '2022-09-26', '2022-10-03',
#                 '2022-10-10', '2022-10-17', '2022-10-24', '2022-10-31', '2022-11-07',
#                 '2022-11-14', '2022-11-21', '2022-11-28', '2022-12-05', '2022-12-12',
#                 '2022-12-19', '2022-12-26'
#         ]
#     },
#     "twenty3": {
#         "months":
#             {
#             "m1":['2023-01-02', '2023-01-09', '2023-01-16', '2023-01-23', 
#             '2023-01-30'],
#             "m2":['2023-02-06', '2023-02-13', '2023-02-20', '2023-02-27'],
#             "m3":['2023-03-06', '2023-03-13', '2023-03-20','2023-03-27'],
#             "m4":['2023-04-03', '2023-04-10', '2023-04-17', '2023-04-24'],
#             "m5":[ '2023-05-01', '2023-05-08', '2023-05-15', 
#             '2023-05-22', '2023-05-29'],
#             "m6":['2023-06-05', '2023-06-12', '2023-06-19', '2023-06-26'],
#             "m7":['2023-07-03', '2023-07-10','2023-07-17', '2023-07-24', '2023-07-31'],
#             "m8":['2023-08-07', '2023-08-14', '2023-08-21', '2023-08-28'],
#             "m9":['2023-09-04', '2023-09-11', '2023-09-18', '2023-09-25'],
#             "m10":['2023-10-02', '2023-10-09', '2023-10-16', '2023-10-23', '2023-10-30'],
#             "m11":['2023-11-06', '2023-11-13', '2023-11-20', '2023-11-27'],
#             "m12":['2023-12-04', '2023-12-11', '2023-12-18'],
#         },
#         "all_files":
#         [
#                 '2023-01-02', '2023-01-09', '2023-01-16', '2023-01-23', 
#                 '2023-01-30', '2023-02-06', '2023-02-13', '2023-02-20', '2023-02-27', '2023-03-06', '2023-03-13', '2023-03-20', 
#                 '2023-03-27', '2023-04-03', '2023-04-10', '2023-04-17', '2023-04-24', '2023-05-01', '2023-05-08', '2023-05-15', 
#                 '2023-05-22', '2023-05-29', '2023-06-05', '2023-06-12', '2023-06-19', '2023-06-26', '2023-07-03', '2023-07-10', 
#                 '2023-07-17', '2023-07-24', '2023-07-31', '2023-08-07', '2023-08-14', '2023-08-21', '2023-08-28', '2023-09-04', 
#                 '2023-09-11', '2023-09-18', '2023-09-25', '2023-10-02', '2023-10-09', '2023-10-16', '2023-10-23', '2023-10-30',
#                 '2023-11-06', '2023-11-13', '2023-11-20', '2023-11-27', '2023-12-04', '2023-12-11', '2023-12-18', '2023-12-25'
#         ]
#     },
#     "twenty4": {
#         "months":
#             {
#             "m1":['2024-01-01', '2024-01-08', '2024-01-15', '2024-01-22', 
#             '2024-01-29'],
#             "m2":['2024-02-05', '2024-02-12', '2024-02-19', '2024-02-26'],
#             "m3":['2024-03-04', '2024-03-11', '2024-03-18','2024-03-25'],
#             "m4":['2024-04-01', '2024-04-08', '2024-04-15', '2024-04-22', '2024-04-29'],
#             "m5":[ '2024-05-06', '2024-05-13', '2024-05-20', 
#             '2024-05-27'],
#             "m6":['2024-06-03', '2024-06-10', '2024-06-17', '2024-06-24'],
#             "m7":['2024-07-01', '2024-07-08','2024-07-15', '2024-07-22', '2024-07-29'],
#             "m8":['2024-08-05', '2024-08-12', '2024-08-19', '2024-08-26'],
#             "m9":['2024-09-02', '2024-09-09', '2024-09-16', '2024-09-23', '2024-09-30'],
#             "m10":['2024-10-07', '2024-10-14', '2024-10-21', '2024-10-28'],
#             "m11":['2024-11-04', '2024-11-11', '2024-11-18', '2024-11-25'],
#             "m12":['2024-12-02', '2024-12-09', '2024-12-16', '2024-12-23', '2024-12-30'],
#         },
#         "all_files":[
#                 '2024-01-01', '2024-01-08', '2024-01-15', '2024-01-22', 
#                 '2024-01-29', '2024-02-05', '2024-02-12', '2024-02-19', '2024-02-26', '2024-03-04', '2024-03-11', '2024-03-18', 
#                 '2024-03-25', '2024-04-01', '2024-04-08', '2024-04-15', '2024-04-22', '2024-04-29', '2024-05-06', '2024-05-13', 
#                 '2024-05-20', '2024-05-27', '2024-06-03', '2024-06-10', '2024-06-17', '2024-06-24', '2024-07-01', '2024-07-08', 
#                 '2024-07-15', '2024-07-22', '2024-07-29', '2024-08-05', '2024-08-12', '2024-08-19', '2024-08-26', '2024-09-02', 
#                 '2024-09-09', '2024-09-16', '2024-09-23', '2024-09-30', '2024-10-07', '2024-10-14', '2024-10-21', '2024-10-28',
#                 '2024-11-04', '2024-11-11', '2024-11-18', '2024-11-25', '2024-12-02', '2024-12-09', '2024-12-16', '2024-12-23', '2024-12-30'    
#         ]
#     }
# }